...
const names = lu => lu.map(id => P[id].name), line = (i, k) => { const r = res.rows[i]; return `${String(k + 1).padStart(2)}  ROI ${r.roi >= 0 ? "+" : ""}${r.roi.toFixed(0)}%${r.se ? " ±" + r.se.toFixed(0) : ""}  proj ${r.projFP.toFixed(1)} (pct ${(100 * r.rProj).toFixed(0)})  own ${r.own.toFixed(0)}  sal ${r.sal}  dup ${r.dupN}  ${r.stack.padEnd(6)} ${r.teams.padEnd(14)} ${names(r.lu).join(", ")}`; };
...
const out = { built: new Date().toISOString(), file, entries: N, pctToFirst: PCT, rake: RAKE, seed: SEED, iters: res.iters, model: "hitSame 0.22 / hitter sigma 0.70", rule: DEFAULT_RULE,
  ...
  field: res.rows.map(r => ({ names: names(r.lu), ids: r.lu.map(id => dkId[id] || null), roi: +r.roi.toFixed(1), cash: +r.cash.toFixed(3), t10: +r.t10.toFixed(3), proj: +r.projFP.toFixed(2), own: +r.own.toFixed(1), sal: r.sal, dup: r.dupN, stack: r.stack, score: r.score > -1e8 ? +r.score.toFixed(1) : null })) };
function pickRow(i) { const r = res.rows[i]; return { roi: +r.roi.toFixed(1), se: r.se != null ? +r.se.toFixed(1) : null, cash: +r.cash.toFixed(3), t10: +r.t10.toFixed(3), proj: +r.projFP.toFixed(2), projPct: +(100 * r.rProj).toFixed(0), own: +r.own.toFixed(1), sal: r.sal, dup: r.dupN, stack: r.stack, teams: r.teams, names: names(r.lu), ids: r.lu.map(id => dkId[id] || null) }; }
