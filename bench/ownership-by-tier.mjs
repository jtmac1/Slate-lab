...
//   node bench/ownership-by-tier.mjs [sport] [from]
import { spearman } from "../src/engine/select.mjs";
import { listPost, readPost } from "./post-store.mjs";

const sport = (process.argv[2] || "mlb").toLowerCase(), FROM = process.argv[3] || "";
...
const size = n => n < 300 ? "<300" : n < 1500 ? "300-1.5K" : n < 10000 ? "1.5K-10K" : n < 100000 ? "10K-100K" : "100K+";

const rows = [];
for (const f of files) {
  const j = readPost(f), c = j.contest;
  const P = j.players.filter(p => p.aown > 0 && p.proj > 0 && p.sal > 0);
  if (P.length < 20) continue;
  const own = P.map(p => 100 * p.aown), proj = P.map(p => p.proj), val = P.map(p => 1000 * p.proj / p.sal), pown = P.map(p => 100 * p.pown), roi = P.map(p => p.aroi ?? -1);
  const sorted = own.slice().sort((a, b) => b - a), mass = own.reduce((s, x) => s + x, 0);
  const chalk = P.filter(p => p.pown > 0).sort((a, b) => b.pown - a.pown).slice(0, 10);
  rows.push({ tier: tier(c.fee), size: size(c.entries), ownProj: spearman(own, proj), ownVal: spearman(own, val), ownPown: spearman(own, pown), ownRoi: spearman(own, roi), pownRoi: spearman(pown, roi),
    top10: 100 * sorted.slice(0, 10).reduce((s, x) => s + x, 0) / mass, maxOwn: sorted[0], chalkMult: mean(chalk.map(p => p.aown / p.pown)) });
}
console.log(`${rows.length} ${sport.toUpperCase()} contests with player data${FROM ? " from " + FROM : ""}`);
...
table("by field size", "size", ["<300", "300-1.5K", "1.5K-10K", "10K-100K", "100K+"]);
