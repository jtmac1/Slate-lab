...
const A = parseCSV(fs.readFileSync(path.join(dir, "actuals.csv"), "utf8")), act = {}; for (const r of A.slice(1)) if (r.length > 6 && r[6] !== "") act[nrm(r[0])] = +r[6];
...
console.log(`generated field (build): mean actual ${mean(B.field.map(r => r.names.reduce((s, n) => s + (act[nrm(n)] || 0), 0))).toFixed(1)} (proj ${mean(B.field.map(r => r.proj)).toFixed(1)}), duplicated ${(100 * B.field.filter(r => r.dup > 0).length / B.field.length).toFixed(0)}%, ownsum ${mean(B.field.map(r => r.own)).toFixed(0)} vs real ${mean(entries.map(e => e.own)).toFixed(0)}`);
...
for (const p of B.picks) { const fp = p.names.reduce((s, n) => s + (act[nrm(n)] || 0), 0), copies = sigReal[key10(p.names)] || 0; console.log(`  #${p.rank} sim ROI ${p.roi >= 0 ? "+" : ""}${p.roi.toFixed(0)}%  ${fp.toFixed(1)} FP -> finish ${finishOf(fp)}/${N}, would pay ${payoutAt(fp) > -100 ? (payoutAt(fp) >= 0 ? "+" : "") + payoutAt(fp).toFixed(0) + "%" : "-"}, copies already in field ${copies}  ${p.stack} ${p.teams}`); }
