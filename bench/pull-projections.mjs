...
const header = "Player,Salary,Position,Bat Pos.,Team,Opponent,Projection,Value,Ownership %,Std Dev,Hand,Confirmed,Team Total,PPD,HR %,Top Pitcher %,Top Value %,DK ID";
...
async function check() {
  ...
  if (fs.existsSync(latest)) {
    const old = {}; for (const r of parseCSV(fs.readFileSync(latest, "utf8")).slice(1)) if (r.length > 8) old[r[0]] = { proj: +r[6], own: +r[8], conf: r[11] };
