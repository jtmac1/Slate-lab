...
/* ---------- Data Hub ---------- */
function renderHub() {
  $("#ctl").innerHTML = `<div class="ctl">${commonCtl()}<div class="f"><label>Projections</label><label class="btn sec" style="cursor:pointer">Load Projections CSV<input type="file" id="fileProj" accept=".csv" hidden multiple></label></div>${F().sport === "mlb" ? `<div class="f"><label>Top stacks</label><label class="btn sec" style="cursor:pointer">Load Topstacks CSV<input type="file" id="fileTs" accept=".csv" hidden></label></div>` : ""}<div class="f"><label>&nbsp;</label><button class="btn ghost" id="btnSample">Load sample slate</button></div><div class="stamp">${S.projWhen ? "Projections loaded: " + esc(S.projWhen) : ""}${S.share ? "<br>Top stacks ✓" : ""}</div></div>`;
  wireCommon();
...
function renderSim() {
  ...
  $("#ctl").innerHTML = `<div class="ctl">${commonCtl()}
    ...
    <div class="f"><label>&nbsp;</label><label class="btn sec" style="cursor:pointer">Upload Lineup File 📄<input type="file" id="fileLu" accept=".csv" hidden></label></div>
...
function mainSim() {
  ...
  if (t === "lineups") {
    ...
    $("#expRes").addEventListener("click", () => exportCSV("pre-contest-lineups.csv", ["Simulated ROI", "Score", "Projected FP", "OwnSum", "Stack", "Stack Type", "Win%", "Top 10%", "Cash%", "Dupes", "Salary"].concat(f.slots), rows.map(r => [r.roi.toFixed(1) + "%", r.score > -1e8 ? r.score.toFixed(1) : "", r.proj.toFixed(2), r.own.toFixed(1) + "%", r.teams, r.type, r.win.toFixed(3), r.t10.toFixed(2), r.cash.toFixed(2), r.dupN, r.sal].concat(r.lu.map(id => P[id].name)))));
...
function renderModal() {
  ...
  if (k === "teams") {
    ...
    html = wrap("Entry Manager", (S.dk.entries.length ? `<div class="crow" style="background:#0d2a1a"><span style="color:var(--red);font-size:16px">♥</span><b>${favs.length} Favorite Lineups</b><div class="grow"></div><span class="hint">Select the contest(s) you want to apply these favorited lineups to.</span><span class="ok">✓</span></div>` : `<div class="note" style="display:flex;align-items:center;gap:12px"><div><b>Upload CSV</b>You need to upload your DraftKings CSV entry file to add lineups to the Entry Manager.</div><div class="grow"></div><label class="btn" style="cursor:pointer">Upload CSV<input type="file" id="fileDk" accept=".csv" hidden></label></div><div class="hint">Don't have the CSV? On DraftKings, open the contest lobby and use "Export lineups to CSV" or download the entries file.</div>`) +
      (S.dk.entries.length ? `<div style="display:flex;align-items:center;gap:12px"><b>My Contests</b><span class="hint">${esc(S.dk.name)}</span><label class="btn ghost" style="cursor:pointer">Edit CSV<input type="file" id="fileDk" accept=".csv" hidden></label><div class="grow"></div><button class="btn ghost" id="emClear">Clear all lineups 🗑</button></div><div style="display:flex;gap:16px;align-items:center;font-size:12.5px">Sort: <label><input type="radio" name="emsort" value="fee"${S.dk.sort === "fee" ? " checked" : ""}> Entry Fee</label><label><input type="radio" name="emsort" value="entries"${S.dk.sort === "entries" ? " checked" : ""}> Entries</label></div>` +
